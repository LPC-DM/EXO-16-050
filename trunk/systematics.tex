...
A 4\% rate uncertainty due to the imperfect knowledge of the CA15 jet energy scale~\cite{jec} is assigned to all the processes obtained from simulation.
...
The \MET trigger efficiency is parametrized as a function of \ptmiss. The uncertainty in its measurement is 1\% and is included in the fit as a rate uncertainty.
An additional systematic uncertainty in the the \MET trigger efficiency is applied that is due to the muon multiplicity in the event.  Differences on the order of 2\% are observed between single-muon and dimuon events at lower $U$ values for those processes whose prediction relies on data events in the single-muon and dimuon CRs (\ttbar, W+jets, and Z+jets production). As these uncertainties depend on the momentum of the identified muon, they can change the shape of the $U$ distribution and are thus treated as shape uncertainties. 
